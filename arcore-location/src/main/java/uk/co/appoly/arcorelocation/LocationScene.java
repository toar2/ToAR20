...
public class LocationScene {
    ...
    public void clearSpecificMarker(LocationMarker lm){
        if(lm.anchorNode != null){
            lm.anchorNode.getAnchor().detach();
            lm.anchorNode.setEnabled(false);
            lm.anchorNode = null;
    ...
    public void clearAllExceptThisMarker(LocationMarker lm){ //needs testing
        for(Iterator<LocationMarker> lmIter = mLocationMarkers.iterator(); lmIter.hasNext();){
            LocationMarker curLocMarker = lmIter.next();
            if(lm == curLocMarker){
                continue;
            }
            else {
                curLocMarker.anchorNode.getAnchor().detach();
                curLocMarker.anchorNode.setEnabled(false);
                curLocMarker.anchorNode = null;
            }
            lmIter.remove();
        }
    }

    public void clearAllExceptTheNonTaggedMarkers(){    //needs testing. Tagged means Model Markers
        for(Iterator<LocationMarker> lmIter = mLocationMarkers.iterator(); lmIter.hasNext();){
            LocationMarker curLocMarker = lmIter.next();
            if (curLocMarker.tag == "") {
                continue;
            }
            if (curLocMarker.anchorNode != null) {
                curLocMarker.anchorNode.getAnchor().detach();
                curLocMarker.anchorNode.setEnabled(false);
                curLocMarker.anchorNode = null;
            }
    ...
    public void clearAllNonTaggedMarkers(){ //non tag means places markers
        for(Iterator<LocationMarker> lmIter = mLocationMarkers.iterator(); lmIter.hasNext();){
            LocationMarker curLocMarker = lmIter.next();
            if (curLocMarker.tag != "") {
                continue;
    ...
    public ArrayList<LocationMarker> findMarkersByTag(String tag){
        ArrayList<LocationMarker> locationMarkerArrayList = new ArrayList<>();
        for (LocationMarker curLocMarker : mLocationMarkers){
            if(curLocMarker.getTag() == tag){
                locationMarkerArrayList.add(curLocMarker);
            }
        }
        return locationMarkerArrayList;
    }
    /**
            ...
            for (int i = 0; i < mLocationMarkers.size(); i++) {
                try {
                    ...
                    if (renderDistance != markerDistance)
                        heightAdjustment += 0.01F * (cappedRealDistance - renderDistance);//0.005
                    ...
                    /*if(mLocationMarkers.get(i).anchorNode.getDistance()<600){
                        //mLocationMarkers.get(i).anchorNode.setHeight(mLocationMarkers.get(i).getHeight()*2);
                        mLocationMarkers.get(i).anchorNode.setHeight(mLocationMarkers.get(i).anchorNode.getDistance());
